...
#     cursor.execute(
#         f"INSERT INTO users (name, phone_number, role, city) VALUES ('{user.name}', '{user.phone_number}', 0, '{user.city}')")
#     conn.commit()
#
...
#     cursor.execute(
#         f"UPDATE auth SET token='{token}' WHERE phone_number='{auth.phone_number}' AND verify_code='{auth.verify_code}'")
#     conn.commit()
#
